...
gold (migration 036, populado por `core.kg.gold`). SUBSTITUI e DELETA o caminho
legado `hypergraph_catalog`/`load_all_hypergraphs` — não há mais dispatcher nem
flag `CATALOG_BACKEND`: SQL é o único backend.
...
def _deadline_display(row: dict) -> str:
    """dd/mm/yyyy — único formato que `core.kg.schema.parse_deadline` lê; a coluna
    SQL é ISO."""
    d = _to_date(row.get("deadline"))
...
def _load_search_snapshot(conn) -> dict:
    ...
    from core.services.company_chunks import parse_vec
...
def _get_search_snapshot() -> dict:
    ...
    from core.services.company_chunks import get_dsn
