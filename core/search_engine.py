...
from config import SILVER_ENRICHED_DIR, CHROMA_DB_DIR as CHROMA_PATH, MODELS_DIR
ENRICHED_FILE = SILVER_ENRICHED_DIR / "editais_enriched.parquet"

# Modelo de Embedding (mesmo usado no ETL)
# Usa modelo fine-tunado local se existir (models/fomento_bert/), senão usa HuggingFace base.
...
class HybridSearchEngine:
    ...
    def __init__(
        self,
        enriched_file: str = None,
        chroma_path: str = "chroma_db",
    ):
        self.enriched_file = Path(enriched_file) if enriched_file else ENRICHED_FILE
        self.chroma_path = Path(chroma_path)

        logger.info("Inicializando HybridSearchEngine...")

        # Carrega dados enriquecidos (metadados + search_document)
        logger.info(f"Carregando dados de {self.enriched_file}...")
        self.df_silver = pd.read_parquet(self.enriched_file)
    ...
    def _build_chroma_where(self, filters: dict) -> Optional[dict]:
        ...
        if "tipo_fluxo" in filters and filters["tipo_fluxo"]:
            tipo_fluxo = filters["tipo_fluxo"]
            if isinstance(tipo_fluxo, list):
                if len(tipo_fluxo) == 1:
                    conditions.append({"tipo_fluxo": {"$eq": tipo_fluxo[0]}})
                else:
                    conditions.append({"tipo_fluxo": {"$in": tipo_fluxo}})
            else:
                conditions.append({"tipo_fluxo": {"$eq": tipo_fluxo}})

        if "tipo_instrumento" in filters and filters["tipo_instrumento"]:
            tipo_instr = filters["tipo_instrumento"]
            if isinstance(tipo_instr, list):
                if len(tipo_instr) == 1:
                    conditions.append({"tipo_instrumento": {"$eq": tipo_instr[0]}})
                else:
                    conditions.append({"tipo_instrumento": {"$in": tipo_instr}})
            else:
                conditions.append({"tipo_instrumento": {"$eq": tipo_instr}})
    ...
    def _apply_filters(self, filters: dict) -> set[str]:
        """Aplica filtros nos metadados enriquecidos (para BM25 e enriquecimento)."""
        if filters is None:
            filters = {}
        ...
        if "tipo_fluxo" in filters and filters["tipo_fluxo"]:
            tipo_fluxo = filters["tipo_fluxo"]
            if isinstance(tipo_fluxo, str):
                tipo_fluxo = [tipo_fluxo]
            if "tipo_fluxo" in self.df_silver.columns:
                mask &= self.df_silver["tipo_fluxo"].isin(tipo_fluxo)

        if "tipo_instrumento" in filters and filters["tipo_instrumento"]:
            tipo_instr = filters["tipo_instrumento"]
            if isinstance(tipo_instr, str):
                tipo_instr = [tipo_instr]
            if "tipo_instrumento" in self.df_silver.columns:
                mask &= self.df_silver["tipo_instrumento"].isin(tipo_instr)
    ...
    def _enrich_results(
        ...
        for doc_id, score, sem_score, lex_score, chunk_text in ranked_results[:top_k]:
            ...
            result = {
                ...
                "trl_min": int(trl_min) if trl_min is not None and pd.notna(trl_min) else None,
                "trl_max": int(trl_max) if trl_max is not None and pd.notna(trl_max) else None,
...
def main():
    ...
    print("=" * 60)
    print("MOTOR DE BUSCA HÍBRIDO - Radar de Mecanismos de Fomento")
    print("=" * 60)
    ...
    while True:
        ...
        if query.lower() == "/filtros":
            ...
            print("\nFiltros disponíveis:")
            print("  source: FAPESP, FINEP, BNDES, EMBRAPII")
            print("  status: ABERTA, ENCERRADA, FLUXO_CONTINUO, VERIFICAR")
            print("  tipo_fluxo: EDITAL, FLUXO_CONTINUO")
            print("  tipo_instrumento: SUBVENCAO_ECONOMICA, PESQUISA_BASICA, CREDITO_INOVACAO, MATCHING_EMBRAPII...")
            continue
