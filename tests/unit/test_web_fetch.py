...
from radar.core.web import fetch as web_fetch
...
def test_fetch_url_respects_char_limit():
    with patch("radar.core.web.fetch.safe_get", return_value=_fake_response()) as get:
...
def test_fetch_url_larger_limit_returns_more():
    with patch("radar.core.web.fetch.safe_get", return_value=_fake_response()):
...
def test_cache_expires_after_ttl(monkeypatch):
    ...
    web_fetch.clear_cache()
    times = [1000.0]
    with patch("radar.core.web.fetch.time.monotonic", side_effect=lambda: times[0]), \
         patch("radar.core.web.fetch.safe_get", return_value=_fake_response()) as get:
...
def test_web_search_is_cached():
    ...
    with patch("radar.core.web.fetch._ws.web_search", return_value=hits) as ws_mock:
