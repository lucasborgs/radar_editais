...
def _record(*, data_extracao: str | None = "2026-07-19 03:06:41", normative: list[dict] | None = None) -> dict:
    record = {
        "native_id": "37-2026",
...
def test_base_and_amendment_map_without_invented_links_or_order(monkeypatch, tmp_path):
    ...
    assert all(doc.authority_state is AuthorityState.CONTEXTUAL for doc in bundle.documents)
...
@pytest.mark.parametrize("function_name", [
    "_build_chunks_for_edital", "_build_all_silver", "ingest_promoted_edital_task",
])
def test_all_source_docs_paths_attempt_fapesc_bundle_first(function_name):
    source = inspect.getsource(getattr(tasks, function_name))
    assert source.index("_save_fapesc_bundle_if_available") < source.index("source_docs.save")
