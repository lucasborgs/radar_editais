-- procrastinate schema (ADR M8). Generated from procrastinate 3.6.0 on 2026-05-12.
-- See https://procrastinate.readthedocs.io/
--
-- Obtained via: `python -m procrastinate schema --read` (canonical SQL ships at
-- procrastinate/sql/schema.sql inside the installed package).
--
-- WARNING: this schema is NOT idempotent. CREATE TYPE / CREATE TABLE / CREATE INDEX
-- statements will fail on a second apply. Supabase migrations run once on each
-- fresh database (`supabase db reset` from scratch, `supabase migration up` for
-- new files), so this is fine in practice. Do not edit this file when bumping
-- procrastinate — instead, add a new migration with the upstream delta from
