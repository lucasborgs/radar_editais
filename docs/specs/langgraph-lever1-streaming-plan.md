...
5. **Smoke Anthropic ELEVADO a condição de gate** (não recomendação): `resolve_agent_provider` prefere Anthropic quando a key existe — ou seja, **o caminho não testado no spike é o caminho default de produção**. Dobra-se no smoke obrigatório via Docker+Cloudflare Tunnel das TASKs 3/4 (um smoke cobre os dois riscos). Não bloqueia a TASK 2.
