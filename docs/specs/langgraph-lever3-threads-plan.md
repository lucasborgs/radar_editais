...
| T3 EXPLORE (2ª fatia) | ⏳ **PRÓXIMA** — não começada. Carrega saver singleton-por-loop + descobertas A/B (delta-slicing + system idempotente). Ver §TASK 3 |
| T5 robustez (concorrência + purge) | ⏳ PENDENTE (enxuta; **purge assume thread-por-turno — revisão OBRIGATÓRIA antes de prod**) |
