...
1. `feat(rt03-t06a): add GET /source-coverage administrative endpoint`
...
- Lê `source_runs` e `discovered_opportunities` via service-role (sem RLS de usuário final).
...
- Erros sanitizados: sem DSN, query, traceback, URL sensível.
...
77 testes passando (incluindo os 60 existentes de `test_source_coverage_metrics.py` e `test_admin_gate.py`):
