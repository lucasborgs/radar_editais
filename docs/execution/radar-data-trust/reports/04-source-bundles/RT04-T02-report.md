...
| `tests/unit/test_source_bundles_repo.py` | 32 testes de unidade do repositório (stubs, sem DB real) |
...
- **`save(bundle: SourceBundle) -> bool`:**
  ...
  - Erros com outro código (`500`, etc.) ou exceções inesperadas levantam `BundleStorageError` — nunca silenciados como sucesso, sem encadear a exceção bruta do provedor.
...
- **`load(subject_kind: str, subject_id: str) -> SourceBundle | None`:**
  ...
  - Desserializa o JSONB como `SourceBundle.model_validate()` — round-trip completo do contrato. Payload ausente, vazio ou inválido levanta `BundleStorageError` sanitizado.
  - `None` apenas para ausência legítima (sem registro ou Supabase não configurado). Falhas reais levantam `BundleStorageError`, sem causa bruta encadeada.
...
| `TestRealErrors` | 3 | APIError 500 levanta BundleStorageError; exceção inesperada sanitizada; APIError sem code levanta |
| `TestLoad` | 8 | complete rows; empty; payload ausente/vazio; payload inválido; query structure; erro de leitura sanitizado |
...
- `pytest tests/unit/test_source_bundles.py tests/unit/test_source_bundles_repo.py` — **104 passed**
