...
consumindo exclusivamente a API administrativa da T05. A página agora alterna
entre a fila editorial existente e a nova aba de exceções, sem mexer em regra
temporal, promoção/rejeição ou qualquer backend Python.

A interface permite listar, filtrar, abrir detalhes e registrar uma das quatro
decisões permitidas, com retry preservando `review_id` enquanto a mesma
submissão permanece aberta.
...
- Enquanto a mesma submissão permanece aberta, o retry reutiliza o mesmo
  `review_id`.
- O painel ignora respostas obsoletas de lista e detalhe usando IDs de
  requisição e limpeza de efeito.
- Fechar o modal invalida a atualização pendente do detalhe.
