...
- **Regressão corrigida (5a5cbc5d2):** `src/radar/core/eval/e2e_health.py` importava
  ...
  `from tests.helpers.gold_projection` — sem `sys.path` hack ou `PYTHONPATH` extra.
- Testes direcionados (`provenance`, `e2e_health`, harness, golden): **71 passed**.
- `ruff check` sobre todo Python versionado: **verde**.
- `provenance` executada duas vezes: agregados idênticos (exact 2/6,
  document_only 2/6, unresolved 2/6; faithfulness 4/4 entre resolvidos).
- `e2e_health` executada duas vezes: agregados idênticos; state presente e
  producer completo em 1.0; `operational_error=0.0`.
- `pytest -q` completo (suíte integral, sem exclusões): **1400 passed, 64 skipped,
  1 failed**. A única falha é `test_card_has_exact_key_set` — `_row_to_card`
  expõe `'provenance'` mas `CARD_KEYS` não a lista. Pré-existente em
  `37f34a74d` (os 4 arquivos — `entity_catalog`, `chunker` e seus testes — não
  diferem entre base e branch). Chunker: 10 passed.
...
- tests/unit/test_gold_equivalence.py, test_gold_provenance_*.py (5 testes corrigidos)
