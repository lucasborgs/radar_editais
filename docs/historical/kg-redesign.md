...
3. **Card v2 estendido no backend.** `edital_card(full)` passou a surfacar `kind`/`aperture`/`macro_temas`/`mecanismo[]` (vinham no nó, não iam ao card). `_curated_card` monta o payload de programa/investimento a partir das **propriedades do nó** (url/ticket/estágio/lead-follow — por-nó, PR4.1) + arestas.
