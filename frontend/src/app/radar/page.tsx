...
function ComparisonPanel({ editais, onRemove, onStartWriting }: { editais: RadarMatchesResponse["matched_editais"]; onRemove: (id: string) => void; onStartWriting: (edital: RadarMatchesResponse["matched_editais"][number]) => void }) {
  ...
  const rows = [
    { label: "Prazo", render: (e: RadarMatchesResponse["matched_editais"][number]) => e.prazo ? `${e.prazo} · ${urgencyLabel(e.prazo)}` : urgencyLabel(e.prazo) },
...
export default function RadarPage() {
  ...
  return <main className="mx-auto min-h-screen max-w-4xl px-4 py-8 sm:py-10"><header className="mb-8 flex flex-wrap items-end justify-between gap-3"><div><p className="text-sm font-semibold text-primary">Radar</p><h1 className="text-2xl font-semibold text-content-primary">Oportunidades para {profile.nome}</h1><p className="mt-1 max-w-2xl text-sm text-content-secondary">A ordem reflete afinidade entre trechos do seu perfil e do escopo de cada oportunidade. Confira as evidências e a elegibilidade antes de decidir.</p></div><button type="button" onClick={() => void loadRadar()} disabled={loading} className="rounded-lg border border-border px-3 py-2 text-sm font-medium text-content-primary hover:bg-app-bg disabled:opacity-50">{loading ? "Atualizando…" : "Atualizar"}</button></header>
    ...
    {data && hasResults && <><section className="mb-7 rounded-xl border border-border bg-surface p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><h2 className="text-sm font-semibold text-content-primary">Filtrar resultados</h2><p className="text-xs text-content-secondary">{visibleEditais.length} editais, {visibleProgramas.length} programas e {visibleInvestidores.length} investidores exibidos.</p></div><button type="button" onClick={() => setFilters(DEFAULT_RADAR_FILTERS)} className="text-xs font-medium text-primary hover:underline">Limpar filtros</button></div><div className="mt-3 flex flex-col gap-3"><div className="flex flex-wrap gap-2"><span className="self-center text-xs text-content-secondary">Setores</span>{setores.map((setor) => <FilterButton key={setor} active={filters.setores.includes(setor)} onClick={() => setFilters((current) => ({ ...current, setores: current.setores.includes(setor) ? current.setores.filter((item) => item !== setor) : [...current.setores, setor] }))}>{setor}</FilterButton>)}</div><div className="flex flex-wrap gap-2"><span className="self-center text-xs text-content-secondary">Elegibilidade</span>{(["all", "elegivel", "nao_verificada"] as const).map((value) => <FilterButton key={value} active={filters.eligibility === value} onClick={() => setFilters((current) => ({ ...current, eligibility: value }))}>{value === "all" ? "Todos" : value === "elegivel" ? "Confirmada" : "A confirmar"}</FilterButton>)}</div><div className="flex flex-wrap gap-2"><span className="self-center text-xs text-content-secondary">Prazo</span>{(["all", "closing", "soon", "continuous"] as const).map((value) => <FilterButton key={value} active={filters.deadline === value} onClick={() => setFilters((current) => ({ ...current, deadline: value }))}>{value === "all" ? "Todos" : value === "closing" ? "Até 7 dias" : value === "soon" ? "Até 30 dias" : "Contínuo / sem prazo"}</FilterButton>)}</div><div className="flex flex-wrap gap-2"><span className="self-center text-xs text-content-secondary">Ordenar editais</span>{(["affinity", "deadline"] as const).map((value) => <FilterButton key={value} active={filters.order === value} onClick={() => setFilters((current) => ({ ...current, order: value }))}>{value === "affinity" ? "Afinidade" : "Prazo mais próximo"}</FilterButton>)}</div></div></section><div className="flex flex-col gap-9">{visibleEditais.length > 0 && <Trail title="Editais" description="Chamadas com prazo e escopo compatíveis.">{visibleEditais.map((edital) => <MatchedEditalCard key={edital.entity_id} edital={edital} onStartWriting={(source, id) => void startWriting(`${source}:${id}`)} onCompare={() => toggleCompare(edital.entity_id)} isCompared={comparedIds.includes(edital.entity_id)} />)}</Trail>}{visibleProgramas.length > 0 && <Trail title="Programas" description="Iniciativas recorrentes que podem apoiar a jornada.">{visibleProgramas.map((entity) => <MatchedEntityCard key={entity.entity_id} entity={entity} onStartWriting={(id, mode) => void startWriting(id, mode)} />)}</Trail>}{visibleInvestidores.length > 0 && <Trail title="Capital privado" description="Investidores cuja tese se aproxima do seu perfil.">{visibleInvestidores.map((entity) => <MatchedEntityCard key={entity.entity_id} entity={entity} onStartWriting={(id, mode) => void startWriting(id, mode)} />)}</Trail>}{visibleEditais.length === 0 && visibleProgramas.length === 0 && visibleInvestidores.length === 0 && <div className="rounded-xl border border-border bg-surface p-5 text-sm"><p className="font-medium text-content-primary">Nenhum resultado corresponde aos filtros atuais.</p><button type="button" onClick={() => setFilters(DEFAULT_RADAR_FILTERS)} className="mt-2 text-primary hover:underline">Limpar filtros</button></div>}</div><ComparisonPanel editais={comparedEditais} onRemove={toggleCompare} onStartWriting={(edital) => void startWriting(`${edital.source}:${edital.edital_id}`)} /></>}
  </main>;
}
