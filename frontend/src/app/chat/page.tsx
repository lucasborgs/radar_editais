 ...
 *    navega para /workspace/{id}. Erro são → toast + volta pra "/".
 *
...
function ChatResolverInner() {
  ...
  // Evita resolver duas vezes (StrictMode / re-render).
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Sem alvo ou sem perfil → front-door (onde o perfil é construído).
      if (!editalId || !profile) {
        router.replace("/");
        return;
      }
      ...
      if (!token) {
        // Anônimo: o gate de login vive na 1a; mandamos pro login.
