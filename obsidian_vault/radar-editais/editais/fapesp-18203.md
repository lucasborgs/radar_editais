---
title: "Chamada de Propostas para o Programa PIPE Jornada Tecnológica Soberania Digital – Fase 1"
chamada_id: fapesp:18203
status: ABERTA
deadline: 24/06/2026
mechanism: subvencao
trl_min: 1
trl_max: 9
n_pdfs: 0
n_facts: 0
link: https://fapesp.br/18203
tags:
  - fapesp
  - edital
  - aberta
  - mecanismo/subvencao
  - trl/trl_pesquisa
  - trl/trl_prototipo
  - trl/trl_industrial
  - ano/desconhecido
---

# 🟢 Chamada de Propostas para o Programa PIPE Jornada Tecnológica Soberania Digital – Fase 1

> A chamada visa apoiar a execução de pesquisa científica e tecnológica inovativa em pequenas empresas do estado de São Paulo, com foco em desafios tecnológicos de interesse para a empresa ou mercado. O financiamento é destinado a projetos que representem um desafio tecnológico e que tenham potencial para gerar inovações.

## Fonte
- [[radar-editais/fontes/fapesp|FAPESP]]

## Informações

| Campo | Valor |
|---|---|
| Status | 🟢 ABERTA |
| Prazo de envio | 24/06/2026 |
| Mecanismo | Subvenção (não reembolsável) |
| Valor do projeto | — a R$ 500,000 |
| TRL aceito | 1 a 9 |
| Entidades elegíveis | empresas |
| PDFs disponíveis | 0 |
| Fatos extraídos | 0 |
| Link | [https://fapesp.br/18203](https://fapesp.br/18203) |

## Requisitos Principais

- A proposta deve ser submetida por um Pesquisador Responsável que seja sócio da empresa.
- A empresa deve ter até 250 empregados e sede no estado de São Paulo.
- O projeto deve ter duração máxima de 12 meses.
- O valor máximo de financiamento por projeto é de R$ 500.000,00.
- O projeto deve incluir um desafio tecnológico relevante.

## Fatos-chave

- Financiamento de até R$ 500.000,00 por projeto.
- Apoio a inovações em tecnologia com foco em Soberania Digital.
- Duração do projeto de até 12 meses.
- A proposta deve ser submetida por um Pesquisador Responsável vinculado à empresa.
- A chamada está aberta até 24/06/2026.
