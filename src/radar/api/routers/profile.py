...
from radar.api.library_routes import MAX_UPLOAD_MB
...
from radar.core.ingestion.profile_inference import infer_financiamento
from radar.core.services.content_library import extract_document_text, get_item, get_workspace_id
...
def extract_profile(request: Request, req: ProfileExtractRequest):
    ...
    Quando `AGENT_PROFILE_EXTRACTOR_DEFAULT_ENABLED=true`, roda o caminho
    agêntico experimental com investigação multi-página; BrasilAPI possui gate
    separado. Default OFF — segue o pipeline determinístico (1 fetch da home +
    1 chamada LLM). Sem workspace, o seletor é binário por env.
    """
    url = req.url.strip()
