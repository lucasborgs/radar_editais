...
CLI:
    python -m radar.core.vocab_lint              # coleta + proposta LLM + relatório
    python -m radar.core.vocab_lint --dry-run    # só coleta + imprime evidência (sem LLM)
    python -m radar.core.vocab_lint --out <dir>  # diretório de saída (default data/vocab_lint/)
"""
...
from radar.domain.vocabulary import canonicalize_themes
...
def build_report(evidence: dict, proposal: dict | None) -> str:
    ...
    lines += ["## 5. Checklist de aplicação", "",
          ...
          "4. [ ] Reingestão deliberada: `python -m radar.core.kg.gold --no-skip`.",
          "5. [ ] Gate de não-regressão: `python -m radar.core.eval matching --no-push` "
