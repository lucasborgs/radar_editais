...
from radar.core.config import FINEP_PDFS_DIR
from radar.core.reflection_service import _auto_memory_write_enabled, load_active_insights
from radar.core.retrieval.retriever import (
    format_chunks_for_prompt,
    retrieve_chunks,
)
from radar.core.services.content_library import get_item, mark_items_referenced
...
# PDFs a ignorar (deve permanecer sincronizado com `radar.core.tasks._SKIP_KEYWORDS`).
...
# request_user_info) são registradas via radar.core.llm.agent_tools.build_writing_tools.
...
class WritingSession:
    ...
    def __init__(
        ...
        if self.mode == "pitch":
            ...
            from radar.core.kg.temporal import render_temporal_block
    ...
    def _load_documents(self, edital_id: str) -> str:
        ...
        A função vive em `radar.pipeline.adapters.finep` (fonte de verdade do dedup
        ...
        from radar.pipeline.adapters.finep import _filter_to_latest_versions
    ...
    def _build_investidor_card_context(self) -> str:
        ...
        """
        from radar.core.kg import entity_catalog  # lazy: evita custo no boot do módulo
        try:
    ...
    def _trim_thread_history(self, thread_id: str) -> None:
        ...
        from radar.core.llm.agent_graph import WR_SYSTEM_MSG_ID, trim_thread_history
        try:
            trim_thread_history(
                thread_id,
    ...
    def generate_full_proposal(self, sections: list[str] | None = None,
        ...
        """
        from radar.core.llm.agent_graph import _try_parse_generation_json, run_generation_turn
        from radar.core.llm.agent_runtime import resolve_agent_provider
        from radar.core.llm.agent_tools import build_writing_tools
