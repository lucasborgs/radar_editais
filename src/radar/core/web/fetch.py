...
invertido: pesquisa dependendo de internals de uma tool de perfil). Agora:
  ...
  • `web_search(query, k)`  → list[SearchHit] — wrapper canônico sobre
                             radar.core.web_search, com o mesmo cache TTL.
...
from radar.core import web_search as _ws
from radar.core.infra.net_guard import safe_get
from radar.core.web_search import SearchHit, WebSearchError  # re-export p/ chamadores
...
def web_search(query: str, k: int = 5) -> list[SearchHit]:
    """Busca web canônica (wrapper sobre radar.core.web_search), com cache TTL.
