...
Roda o MESMO `_triage` de produção (`radar.core.ingestion.opportunity_discovery`) sobre
...
Mudou prompt/input/modelo da triagem → rode `python -m radar.core.eval triage`
...
def task(*, item: Any, **_) -> dict:
    inp = get_input(item)
    from radar.core.ingestion.opportunity_discovery import _make_client, _triage
