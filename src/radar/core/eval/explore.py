"""Golden dos casos motivadores do Explorar (editais ativos).

Investidores ficaram fora do escopo (spec product-scope-catalog-deactivation.md)
e não têm mais golden case. Default hermético valida rota. Com
``EVAL_EXPLORE_CONNECTED=true`` executa o pipeline real (retrieval, síntese/
agente e catálogo) e julga conteúdo contra o golden semântico do NotebookLM,
com a adaptação temporal da FINEP.
"""
...
def eval_tool_contract(*, output, expected_output, **_) -> Evaluation | None:
    ...
    if route in {"EDITAL_FACT", "EDITAL_FACT_ENUMERATIVE"}:
        ...
        # Entidades ativas (ICTs) usam o toolset de leitura do grafo; investidores
        # foram desativados das superfícies ativas e não têm tool dedicado.
        expected = "get_node_neighborhood or search_entities"
        acceptable = {"get_node_neighborhood", "search_entities", "list_icts"}
    else:
        return None
...
SUITE = Suite(
    name="explore",
    description="Rota hermética ou E2E conectado dos casos factuais golden (editais ativos).",
    load_data=load_data,
    task=task,
